    ...
    {   
        ...
        {            
            ...
            function fixeScroLi() {
                ...
                if ( ($(document).scrollTop() + top > content.offset().top + top ) && $(document).scrollTop() + $("#WS-ScroLi").height() < content.offset().top + content.height() )  {
                    app.css('top', `${top}px`).addClass('fixed').removeClass('absolute');
                } else {
                    app.css('top', `${content.offset().top + top + 80}px`).removeClass('fixed').addClass('absolute');
                }
            }
            ...
            function createScroLiDOM() {
                ...
                $.each(options.sections, (i)=> {
                    $('#WS-ScroLi').append(`
                        <span data-target="${options.sections[i][0]}" class="item-${i+1}">
                            <i class="icon ${options.sections[i][1]}"></i>
                            <div class="time-point">${options.sections[i][2]}<span>${options.sections[i][3]}</span></div>
